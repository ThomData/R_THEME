...
## LOOP on the k samples	  
    ...
    ## browser() ## Checker les sorties
    ## Comment les stocker les concatener ensuite
		for(j in 1:nbeq){
			if(i==1){
				eq[[j]]<-0
				R2CV[[j]]<-0
			  }
		  if(length(listsamples[[i]])==1){eq[[j]]<-eq[[j]]+abs(as.vector(mypred$Ypred[[j]])-unlist(Xtotioptim[resE$rEq[[j]][1]]))}
			if(length(listsamples[[i]])>1){eq[[j]]<-eq[[j]]+apply(abs(mypred$Ypred[[j]]-(Xtotioptim[resE$rEq[[j]][1]])[[1]]),2,mean)}
